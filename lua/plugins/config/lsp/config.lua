...
map('n', '<c-k>', vim.lsp.buf.signature_help, { desc = 'Displays signature information about the symbol under the cursor' })
...
map('n', '<space>q', function() vim.diagnostic.setqflist({ open = true }) end, { desc = 'Add all diagnostics to the quickfix list' })
map('n', '<space>e', function() vim.diagnostic.open_float() end, { desc = 'Show diagnostics in a floating window' })
...
map('n', '<space>wl', function()
  vim.notify(vim.inspect(vim.lsp.buf.list_workspace_folders()), vim.log.levels.INFO, { render = 'default', title = 'Workspace folders' })
