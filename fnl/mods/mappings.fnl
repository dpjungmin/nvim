...
(map! [n] :<leader>ss (.. ":mks! " vim.g.sessiondir "/")
      {:desc "Make session" :silent false})

(map! [n] :<leader>so (.. ":so " vim.g.sessiondir "/")
      {:desc "Open session" :silent false})

(map! [n] :<leader>w :<cmd>update<cr>
      {:desc "Write only when the buffer has been modified"})

(map! [n] :<leader>q :<cmd>x<cr>
      {:desc "Write only when changes have been made and quit"})

(map! [n] :<leader>Q :<cmd>xa<cr> {:desc "Write all changed buffers and exit"})
(map! [n] :<leader><leader>q :<cmd>qa!<cr>
      {:desc "Force close all buffers and exit"})

(map! [n] :<leader>y "<cmd>%y<cr>" {:desc "Copy the entire buffer"})
(map! [n] :<leader>cd "<cmd>lcd %:p:h<cr><cmd>pwd<cr>"
      {:desc "Change current working directory locally and print cwd"})

(map! [n] :<leader><leader>x "<cmd>update<cr><cmd>source %<cr>"
...
(map! [n] :<space>ao
      (fn []
        (string.format "m`%so<esc>``" vim.v.count1))
      {:desc "Add a blank line below the current line" :expr true})

(map! [n] :<space>aO
      (fn []
        (string.format "m`%sO<esc>``" vim.v.count1))
      {:desc "Add a blank line above the current line" :expr true})

(map! [n] :<space>ov "<cmd>vs %<cr><c-w>h"
      {:desc "Open current file in a vertical split"})

(map! [n] :<space><space> :a<space><esc>h
      {:desc "Insert a space after the current character"})
...
(map! [i] :<c-u> :<esc>viwUea
      {:desc "Turn the word under the cursor into upper case"})

(each [_ ch (ipairs ["," "." "?" ";" ":" "!"])]
  (map! [i] ch (.. ch :<c-g>u)
        {:desc (.. "Break " ch " into smaller undo units")}))
...
; hold on to the value that is being pasted
...
(map! [n] :<space>ti (fn indent-blankline-toggle []
...
(map! [n] :<space>tm :<cmd>MarkdownPreviewToggle<cr>
...
(map! [n] :<space>? :<cmd>MatchupWhereAmI?<cr>
...
(map! [v] :<leader>s :<cmd>ToggleTermSendVisualSelection<cr>
      {:desc "Send the visually selected text to the terminal"})

(map! [n] :<c-n> ":<c-u>ToggleTerm direction=vertical<cr>"
      {:desc "Toggle $TERM vertically"})

(map! [n] :<space>ot ":<c-u>ToggleTerm direction=tab<cr>"
      {:desc "Open $TERM in a new tab"})
...
(map! [n] :<space>bp :<cmd>BufferLineTogglePin<cr>
      {:desc "Toggle pin for current buffer"})

(map! [n] :<s-l> :<cmd>BufferLineMoveNext<cr>
      {:desc "Move the current buffer to the next buffer position"})

(map! [n] :<s-h> :<cmd>BufferLineMovePrev<cr>
...
(map! [n] :<space>fr "<cmd>Telescope oldfiles<cr>"
      {:desc "Lists most recently opened files"})

(map! [n] :<space>fm "<cmd>Telescope man_pages<cr>"
      {:desc "Lists man page entries"})

(map! [n] :<space>fh "<cmd>Telescope help_tags<cr>"
      {:desc "List available help tags"})

(map! [n] :<space>fH "<cmd>Telescope highlights<cr>"
      {:desc "List available highlights"})

(map! [n] :<space>K "<cmd>Telescope lsp_references<cr>"
      {:desc "Lists LSP references for the work under the cursor"})

(map! [n] :<space>fd "<cmd>Telescope diagnostics<cr>"
