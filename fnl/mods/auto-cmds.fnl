(local auto-cmds
       {:hl-yank [[:TextYankPost
        ...
        :toggle-nu-on-imode [["BufEnter,FocusGained,InsertLeave,WinEnter"
                              "*"
                              "if &nu | set rnu | endif"]
                             ["BufLeave,FocusLost,InsertEnter,WinLeave"
                              "*"
                              "if &nu | set nornu | endif"]]
        :resume-cursor-position [[:BufReadPost
                                  "*"
                                  "lua require('lib')['resume-last-cursor-pos']()"]]
        :toggle-indent-blankline [[:InsertEnter "*" :IndentBlanklineDisable]
                                  [:InsertLeave
                                   "*"
                                   "lua require('lib')['indent-blankline-activate']()"]]
        :format-on-save [[:BufWritePost "*" :FormatWrite]]
        :au-compile-packer [[:BufWritePre
                             :*/nvim/lua/plugins.lua
                             "source <afile> | PackerCompile"]]
        :puppet [["BufNewFile,BufRead" :*.pp "setfiletype puppet"]
                 ["BufNewFile,BufRead"
                  :*.pp
                  "set shiftwidth=2 softtabstop=2 filetype=puppet"]]
